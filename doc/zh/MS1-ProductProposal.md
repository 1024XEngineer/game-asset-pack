...
- LayOut (UI 组件的布局图)：
  - Components (组件，用户可以使用我们预设或custom组件来组织布局):
      - Name
      - Description (如果是custom 组件，需要用户描述出这个组件的形状)
- Style (对这套UI组件的整体风格描述)
- Reference (风格参考图)
- 是否使用Project Context

生成后会为用户展示一张大图，用户觉得不满意可以重新生成，认为满意后会对大图切割出各个组件，用户之后可以针对具体的组件通过prompt进行修改。
...
Tiles Asset包含一组瓦片，用户通过填写表单生成一组可拼接的Tile
表单：
- Asset Name
- Template (Tiles 模版, e.g. 3X5,4X4)
- Description (对这个Tiles的描述)
- Reference (参考图片)
![alt text](/doc/image/tile-example.png)
- 是否使用Project Context

生成后用户可以看到一张大的Tile Set图，用户确认后系统会对其分割出一个个Tile,用户可以在工作台进行拼接。



#### Audio

Audio Asset不同于附属于动画的音效，它属于Asset的一种类型，通常用于生成我们游戏中的BGM、环境音等,Audio Asset可以包含多条Track
创建时需要填写：
- Asset Name
...
生成后用户可以在线播放、倍速来查看效果，也可以进行基本的裁剪操作，后续也可以通过提示词继续对其更改。
如果用户对音频有更复杂的需求，也可以继续新生成Track，通过组合实现更灵活的音频效果。
